...
- **`@destructive` is the heaviest tag and always wins.** If a test would otherwise be `@smoke` / `@sanity` / `@regression` / `@e2e` / `@api` but modifies shared application state, it is tagged **only** `@destructive`.
- **Every `@destructive` test** must have `test.afterEach()` or `test.afterAll()` cleanup that reverts state changes.
...
| `@destructive` | Modifies shared application state — excluded from `npm test`, run via `npm run test:destructive` |

**`@destructive` overrides any other importance tag.** If a test would otherwise be `@smoke` but mutates shared state, it is tagged **only** `@destructive`.
...
Tests tagged `@destructive` modify shared application state (deleting data, changing global settings, resetting configurations). They follow strict rules.

**Cleanup hook is required.** Every `@destructive` test **MUST** use `test.afterEach()` or `test.afterAll()` to revert state changes:
