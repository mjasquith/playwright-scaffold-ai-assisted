...
- **`.cursor/rules/rules.mdc` Constitution is the safety floor.** MUST/SHOULD/WON'T tables are hard stops; they take precedence over any prose, template, or example.
...
- **`.cursor/rules/rules.mdc`** — always-loaded Constitution (MUST/SHOULD/WON'T tables).
